...
3. **The harvest curve** (3-trading-day window, touch-based ceiling, N=2,029):
   ...
   Serve it live, with your own filters, via `query_outcomes(view="harvest")`.
...
## The four surfaces (all views of `query_outcomes`)

**1. `query_outcomes(view="surface")`** — the raw material.
...
**2. `query_outcomes(view="summary")` / `query_outcomes(view="labels")`** — reference brackets, exactly simulated.
...
**3. `query_outcomes(view="harvest")`** — the touch probabilities, live.
...
**4. `query_outcomes(view="exit_rule")`** — your bracket, classified against the surface.
...
1. Start from the exact labels: `query_outcomes(view="summary", horizon="same_day")` and `horizon="3d"` — the two known points in the exit space.
2. Sweep brackets with `query_outcomes(view="exit_rule")` (e.g. targets 20–120%, stops −20 to −60%), recording `ev_bounds` and `heuristic_share` for each.
