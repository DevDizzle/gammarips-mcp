...
> **Last reviewed:** 2026-09-01 (V4 surface — 9 tools, now with `title` + read-only annotations; OAuth 2.1 resource server, `/pro` endpoint)
...
| Date | Change |
|---|---|
| 2026-09-01 | **Truthfulness fixes on two tool responses. No security-surface change.** `query_outcomes` frontier: `open_past_due` counted a window as overdue the moment it closed, but a window closes at midnight ET and `pool-outcomes-refresh` does not run until 17:20 ET, so the tool asserted a stalled pipeline to every paying subscriber every weekday until evening. A row is now past due only once another session has elapsed or it is past 18:00 ET; rows closed today are reported separately as `open_awaiting_fill`. `get_signal_performance` returned `row_count` alone, so a capped response was indistinguishable from a complete one — and because it sorts `scan_date DESC, score DESC`, a truncated response ends mid-scan_date on SCORE and its oldest date is a highest-score-only slice. **Response shape (additive):** `matched_rows` + `truncated`, and the note refuses to look complete when it is not. Audited the other row paths: `query_outcomes` already reported truncation; `historical.py` caps at 500 over a 48-row ledger and cannot bind. `SERVER_VERSION` 4.5.0. No change to read-only status, parameterization, tiering, clamps, or the no-same-day-pick guarantee. |
| 2026-09-01 | **Tool `title` + MCP annotations on all 9 tools (claude.ai Connectors Directory gate).** The real `tools/list` carried NO titles and NO annotations; the `readOnlyHint` that existed served only the stateless JSON-RPC path and the Smithery server card. `_TOOL_TITLES` in `src/server.py` is now the single source for both surfaces so they cannot drift. All nine are read-only queries over fixed backends: `readOnlyHint=true`, `destructiveHint=false`, `idempotentHint=true`, `openWorldHint=false`. Also pinned `mcp>=1.25.0,<2` — mcp 2.x renamed FastMCP to MCPServer and an unbounded pin killed rev 00051-b25 on import (production unaffected; the prior revision kept 100% of traffic). `SERVER_VERSION` 4.4.0. Metadata only: no tool, data, clamp, tiering, or leakage-view change. |
