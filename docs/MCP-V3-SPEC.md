...
| `get_historical_performance` | Cohort-aware: default to the live `V7_1_TILTED_GIGO` cohort (`LIVE_COHORT_START_DATE=2026-06-26`), explicit `policy_version` param; never mix cohorts in one aggregate. **[Annotated 2026-08-07 — historical spec, do not read the date as current: the live cohort start is now 2026-08-10 and `src/utils/data.py` is the single source of truth. Note this row DID specify a `LIVE_COHORT_START_DATE` from the beginning; the implementation shipped filtering on the policy LABEL alone and never applied the date, so disowned cohorts were served as live receipts until 2026-08-07. The spec was right and the code did not match it.]** |
